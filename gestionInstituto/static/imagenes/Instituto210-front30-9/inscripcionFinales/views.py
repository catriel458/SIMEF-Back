...
def alta_materia(request):
    previous_page = request.GET.get('previous', None) 

    if request.method == 'POST':
        form = MateriaForm(request.POST)
        if form.is_valid():
            form.save()
            return render(request, 'materias/alta_materia.html', {'form': form, 'creation_success': True})
    else:
        form = MateriaForm()
...
def editar_materia(request, id):
    ...
    return render(request, 'materias/editar_materia.html', {'form': form, 'edit_success': edit_success})
